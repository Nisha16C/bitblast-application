...
class ManageServers:
    ...
    @app.command()
    @update_sqlite_path
    def remove_server(server_name: str, user: Optional[str] = None,
                      auth_source: Optional[str] = INTERNAL,
                      sqlite_path: Optional[str] = None):
        """Remove a server by name."""
        # What user?
        remove_user = user if user is not None else config.DESKTOP_USER
        print('----------')
        print('Removing server with:')
        print('User:', remove_user)
        ...
        try:
            app = create_app(config.APP_NAME + '-cli')
            with app.test_request_context():
                # Get the SQLite path if it wasn't set
                db_path = sqlite_path if sqlite_path else config.SQLITE_PATH
                # Connect to SQLite database
                conn = sqlite3.connect(db_path)
                cursor = conn.cursor()
                # Get the user's ID based on the email
                cursor.execute("SELECT id FROM user WHERE email = ?", (remove_user,))
                user_row = cursor.fetchone()
                if user_row is None:
                    raise Exception(f"User {remove_user} not found.")
                user_id = user_row[0]
                # Find and remove the server by name for the user
                cursor.execute("SELECT id FROM server WHERE name = ? AND user_id = ?", (server_name, user_id))
                server_row = cursor.fetchone()
                if server_row is None:
                    raise Exception(f"Server '{server_name}' not found for user {remove_user}.")
                server_id = server_row[0]
                # Delete the server entry
                cursor.execute("DELETE FROM server WHERE id = ?", (server_id,))
                conn.commit()
                print(f"Server '{server_name}' successfully removed for user {remove_user}.")
        except Exception as e:
            print(f"Error: {str(e)}")
            if conn:
                conn.rollback()
