<template>
  ...
  <aside class="my-3 overflow-auto border-0 sidenav navbar navbar-vertical navbar-expand-xs border-radius-xl" :class="`${this.$store.state.isRTL
    ...
    <div class="sidenav-header text-center">
      ...
      <router-link class="m-0 navbar-brand" :to="logoRoute">
        <img :src="this.$store.state.darkMode ||
            this.$store.state.sidebarType === 'bg-default'
            ? logoWhite
            : logo
          " class="navbar-brand-img mr-4 h-100" alt="main_logo" />
...
export default {
  ...
  computed: {
    ...
    logoRoute() {
      return this.isAdmin ? '/admin-dashboard' : '/User-dashboard'; // Change routes as needed
    },
  },
